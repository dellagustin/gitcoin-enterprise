...
export class AuthenticationServiceDouble {
    ...
    public async createAuthenticationDataFromCode(code: any, state: any): Promise<IAuthenticationData> {
        void this.lg.log(ELogLevel.Debug, `createAuthenticationDataFromCode: ${code} for state ${state}`)
    ...
    public addState(): string {
        void this.lg.log(ELogLevel.Debug, `adding state`)
    ...
    public getOAUTHLoginURL(state: string): string {
        void this.lg.log(ELogLevel.Debug, `getOAUTHLoginURL for state ${state}`)
    ...
    public keepTheAction(action: string, ipAddress: string) {
        void this.lg.log(ELogLevel.Debug, `keepTheAction: ${action} for ip ${ipAddress}`)
    ...
    private getTestAuthenticationData(michaelsfriendskey: any): IAuthenticationData {
        void this.lg.log(ELogLevel.Debug, `getTestAuthenticationData: for ${michaelsfriendskey}`)
    ...
    private async handleNewToken(michaelsfriendskey: any): Promise<IAuthenticationData> {
        void this.lg.log(ELogLevel.Debug, `handleNewToken: for ${michaelsfriendskey}`)
    ...
    private addAuthenticationData(aD: IAuthenticationData): void {
        void this.lg.log(ELogLevel.Debug, `addAuthenticationData: ${JSON.stringify(aD)}`)
        const allAuthenticationData = this.persistencyService.getAuthenticationData()
        if (allAuthenticationData.filter((entry: IAuthenticationData) => entry.token === aD.token)[0] !== undefined) {
            ...
            void this.lg.log(ELogLevel.Info, `Authentication Data added to Store: ${JSON.stringify(allAuthenticationData)}`)
